...
public class OpenBCIReader : MonoBehaviour
{
    ...
    private enum Stages
    {
        CalibrationRefresh = 0,
        CalibrationSensitivity = 1,
        Play = 2
    }

    private Stages stage = Stages.CalibrationRefresh;
        ...
        switch (calibrationStage)
        {
            ...
            case CalibrationStage.InitialRelaxed:
                ...
                else if (_elapsedSinceInput < 6)
                {
                    ...
                    for (var c = 0; c < _eegChannels.Length; c++) relaxedReadings[c].Add(readings[_eegChannels[c]]);
                }
                else
                {
            ...
            case CalibrationStage.InitialFlexed:
                ...
                else if (_elapsedSinceInput < 6)
                {
                    ...
                    var readings = GetReadings();
                    for (var c = 0; c < _eegChannels.Length; c++) flexedReadings[c].Add(readings[_eegChannels[c]]);
                }
                else
                {
                    // readings complete, move on
            ...
            case CalibrationStage.Calculation:
                ...
                for (var c = 0; c < _eegChannels.Length; c++)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (Math.Abs(failsFlexed - failsRelaxed) < 3)
                        {
    ...
    private double[] GetReadings()
    {
        ...
        {
            double avg = 0;
            for (var i = unprocessedData.GetLength(1) - 1; i > unprocessedData.GetLength(1) - 11; i--)
            {
